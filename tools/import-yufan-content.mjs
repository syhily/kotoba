...
function inferCollection(relativePath) {
  ...
  if (relativePath.includes(`${path.sep}categories.`) || relativePath.includes(`${path.sep}categories.yml`)) {
    return 'categories'
  }
  if (relativePath.includes(`${path.sep}tags.`) || relativePath.includes(`${path.sep}tags.yml`)) {
    return 'tags'
  }
  if (relativePath.includes(`${path.sep}friends.`) || relativePath.includes(`${path.sep}friends.yml`)) {
    return 'friends'
  }
  return 'unknown'
}
