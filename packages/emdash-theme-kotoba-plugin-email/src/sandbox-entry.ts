...
import { buildEmailDetailsUrl, buildZeaburPayload, isDeliveryConfirmed, shouldRetry } from './zeabur.ts'
  ...
  hooks: {
    ...
    'email:deliver': {
      ...
      handler: async ({ message, source }: EmailDeliverEvent, ctx: PluginContext) => {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Email delivery failed (${response.status}) source=${source} detail=${body.slice(0, 300)}`)
        }
        const result = (await response.json().catch(() => ({}))) as { id?: string }
        if (result.id) {
          await ctx.kv.set(buildDeliveryLookupKey({ message: message as ProviderEmailMessage, source }), result.id)
        }
    ...
    'email:afterSend': {
      handler: async ({ message, source }: EmailAfterSendEvent, ctx: PluginContext) => {
        ...
        const zeaburId =
          ((await ctx.kv.get(buildDeliveryLookupKey({ message: message as ProviderEmailMessage, source }))) as
            | string
            | null) || null
        if (!zeaburId) return

        const detailsUrl = buildEmailDetailsUrl(options.endpoint || DEFAULT_ZEABUR_ENDPOINT, zeaburId)
