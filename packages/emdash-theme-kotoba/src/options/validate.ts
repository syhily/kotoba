...
export function validateKotobaOptions(options: KotobaThemeOptions) {
  ...
  if (options.pagination) {
    ...
    if (p.postsPerPage !== undefined)
      assertPositiveInteger(p.postsPerPage, "pagination.postsPerPage");
    ...
    if (p.searchPerPage !== undefined)
      assertPositiveInteger(p.searchPerPage, "pagination.searchPerPage");
  }
}
