import { queryPostsByTag } from '../../../../query/content.ts'
...
export async function GET(context: { params: Record<string, string | undefined> }) {
  ...
  const items = posts
    ...
    .join('')
  const rss = `<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>Tag ${escapeXml(slug)}</title>${items}</channel></rss>`
